...
<template>
  ...
  <a v-if="!media" :href="href" class="group block no-underline text-inherit cursor-pointer break-inside-avoid mb-4">
    <article>
      <div class="bg-black px-7 py-6 rounded-xl transition-all duration-200 group-hover:-translate-y-0.5 group-hover:border-gloss-gray-600 border border-gloss-gray-800 !p-0 overflow-hidden">
        ...
        <div class="pl-[18px] pr-3 pb-5 flex flex-col gap-2.5" :class="heroImage ? 'pt-1' : 'pt-3'">
          <div class="flex items-center justify-between gap-3 font-mono text-xs" :class="heroImage ? '' : 'mb-3'">
            <div class="flex items-center gap-4 min-w-0">
              <Badge :type="type" />
              <time :datetime="date" class="text-white/50 whitespace-nowrap shrink-0">{{ dateLabel }}</time>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <AvatarStack :authors="authors" />
            </div>
          </div>
          <h2 class="m-0 text-xl font-medium text-white/90 leading-tight">{{ title }}</h2>
          <p v-if="excerpt" class="m-0 text-sm leading-[1.4] text-white/60 line-clamp-3">{{ excerpt }}</p>
        </div>
      </div>
    </article>
  </a>
  ...
  <article v-else class="bg-black rounded-xl border border-gloss-gray-800 pl-5 pr-3 pt-3 pb-4 break-inside-avoid mb-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-gloss-gray-600">
    <header class="flex items-center justify-between mb-5 gap-3">
      <div class="flex items-center gap-2 min-w-0">
        <Badge :type="type" />
        <time :datetime="date" class="font-mono text-xs text-white/50 whitespace-nowrap">{{ dateLabel }}</time>
      </div>
      <div class="flex items-center gap-2 shrink-0">
        <AvatarStack :authors="authors" />
      </div>
    ...
    <h2 class="m-0 mb-3 text-xl font-medium leading-tight">
      <a :href="href" class="no-underline text-white/90 hover:text-white hover:underline">{{ title }}</a>
    </h2>
    ...
    <div v-if="excerpt" class="text-[14px] leading-[1.4] text-white/60 [&>p]:m-0 [&>p:not(:last-child)]:mb-2 [&_a]:text-lavender-600 [&_a:hover]:underline">
      <p>{{ excerpt }}</p>
    </div>
  </article>
</template>
